...
public class ApiHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent event, Context context) {
        try {
            switch (event.getResource()) {
                case "/signup":
                    return handleSignUp(event).withHeaders(getCorsHeader());
                case "/signin":
                    return handleSignIn(event).withHeaders(getCorsHeader());
                case "/tables":
                    return handleTables(event).withHeaders(getCorsHeader());
                case "/reservations":
                    return handleReservations(event).withHeaders(getCorsHeader());
                default:
                    return handleTables(event).withHeaders(getCorsHeader());
            }
        } catch (Exception e) {
        ...
        return new APIGatewayProxyResponseEvent().withStatusCode(400).withHeaders(getCorsHeader());
    }
    public APIGatewayProxyResponseEvent handleSignUp(APIGatewayProxyRequestEvent event) {
        ...
        try {
            ...
            return new APIGatewayProxyResponseEvent().withStatusCode(200).withHeaders(getCorsHeader());
        } catch (Exception e) {
            System.out.println(e);
            return new APIGatewayProxyResponseEvent().withStatusCode(400).withHeaders(getCorsHeader()).withBody("Failed to sign up!");
        }
    }
    ...
    public APIGatewayProxyResponseEvent handleSignIn(APIGatewayProxyRequestEvent request) {
        ...
        try {
            ...
            response.setHeaders(getCorsHeader());
        }
    ...
    private Map<String, String> getCorsHeader() {
