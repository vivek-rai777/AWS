...
public class TablesHandler {
    ...
    Map<String,String> headers = new HashMap<>(Map.of( "Access-Control-Allow-Headers","Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token",
    ...
    public APIGatewayProxyResponseEvent getTables(APIGatewayProxyRequestEvent event) {
        ...
        if(table.isPresent()) {
            response.setStatusCode(200);
            response.setBody(new Gson().toJson(table.get()));
        }
        else {
